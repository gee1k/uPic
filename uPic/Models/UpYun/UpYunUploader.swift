...
//            fileName = "uPic_\(Date().milliStamp).png"
        ...
        let saveKey = "/\(config.folder ?? "")/\(fileName)"
        
        // MARK: 加密 policy
        var policyDict = Dictionary<String, Any>()
        policyDict["bucket"] = bucketName
        policyDict["save-key"] = saveKey
        
        let policy = getPolicy(policyDict: policyDict)
        
        // MARK: 加密 signature
        let signatureParams = ["POST", "/\(bucketName)", policy]
        ...
        }, to: "\(url)\(bucketName)", headers: headers).uploadProgress { progress in
