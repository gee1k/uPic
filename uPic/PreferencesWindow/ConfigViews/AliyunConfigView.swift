...
class AliyunConfigView: NSView {

    var data: AliyunHostConfig = AliyunHostConfig()
    ...
    init(frame frameRect: NSRect, data: HostConfig?) {
        ...
        if data != nil {
            self.data = data as! AliyunHostConfig
        }
    ...
    @objc func openTutorial() {
        guard let url = URL(string: "https://blog.svend.cc/upic/tutorials/aliyun_oss") else {
            return
        }
