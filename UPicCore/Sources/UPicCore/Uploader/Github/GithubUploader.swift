...
public class GithubUploader {
    ...
    private static func getRequestConfig(_ config: GithubHostConfig, saveKey: String, data: Data) -> RequestConfig {
        ...
        headers.add(HTTPHeader.authorization("token \(config.token!)"))
        headers.add(HTTPHeader.contentType("application/json"))
        headers.add(HTTPHeader.defaultUserAgent)
        
        var requestConfig = RequestConfig()
        requestConfig.url = "https://api.github.com/repos/\(config.owner!)/\(config.repo!)/contents/\(saveKey.urlEncoded())"
    ...
    internal static func handle(_ ctx: UPicCore, model: HostModel, data: Data, filename: String) {
        guard let config = model.data as? GithubHostConfig, config.isValid() else {
