...
public class HostModel: HandyJSON {
    ...
    public init(_ type: HostType, data: HostConfig?) {
        ...
        if let data = data, let jsonString = data.toJSONString(), let jsonData = jsonString.data(using: .utf8) {
    ...
    public func isValid() -> Bool {
        ...
        if let dataRaw = self.dataRaw, let jsonString = String(data: dataRaw, encoding: .utf8), let hostConfig = HostConfig.deserialize(from: jsonString) {
    ...
    public func getConfig<T: HostConfig>(_ type: T.Type) -> T? {
        guard let dataRaw = dataRaw, let jsonString = String(data: dataRaw, encoding: .utf8) else {
            return nil
        }
