...
struct Regex {
    ...
    // 正则替换
    func replacingMatches(in input: String, with template: String,
...
extension Regex {
    ...
    struct Options: OptionSet {
        // 忽略字母
        static let ignoreCase = Options(rawValue: 1)
        
        // 忽略元字符
        static let ignoreMetacharacters = Options(rawValue: 1 << 1)
        
        // 默认情况下,“^”匹配字符串的开始和结束的“$”匹配字符串,无视任何换行。
        // 使用这个配置，“^”将匹配的每一行的开始,和“$”将匹配的每一行的结束。
        static let anchorsMatchLines = Options(rawValue: 1 << 2)
        
        /// 默认情况下,"."匹配除换行符(\n)之外的所有字符。使用这个配置，选项将允许“.”匹配换行符
        static let dotMatchesLineSeparators = Options(rawValue: 1 << 3)
...
extension Regex {
    ...
    class Match: CustomStringConvertible {
        // 匹配的字符串
        lazy var string: String = .init(describing: self.baseString[self.range])
        
        // 匹配的字符范围
        lazy var range: Range<String.Index> = Range(self.result.range, in: self.baseString)!
        ...
        func string(applyingTemplate template: String) -> String {
